...
def parse_args():
    ...
    add_arg('-i', '--input_file_prefix',
            default='../data/{}.pickle.gz', help='input file prefix')
    add_arg('-t', '--task',
            default='regression', help='task type, regression or classification')
    add_arg('-d', '--dataset',
            default='jdft2d',  help='pymatgen dataset name')
    add_arg('-p', '--property',
            default='exfoliation_en', help='Pridict propertys list')
    add_arg('-ki', '--kf_indices',
            default='0,1,2,3,4', help='kfold indices str, split by "," if use more than one')
    add_arg('-w', '--warm_start',
            default=None, help='warm start file')
    add_arg('-b', '--batch_size',
            default=128, type=int, help='mini batch size')
    add_arg('-n', '--n_works', type=int,
            default=1, help='n works for multiprocessing')
    add_arg('-m', '--max_epochs',
            default=300, type=int, help='Max epochs')
    # cgcnn model parameters
    add_arg('-nc', '--n_conv',
            default='4', type=int, help='n cgcnn convs')
    add_arg('-lr', '--learning_rate',
            default=0.02, type=float, help='learning rate')
    add_arg('-ts', '--test',
            action='store_true', help='predict test or not')
    add_arg('-pm', '--pin_memory',
            action='store_true', help='use pin_memory or not')
    add_arg('-dc', '--disable_cuda',
            action='store_true', help='disable cuda or not')
    add_arg('-dt', '--distributed',
            action='store_true', help='distributed or not')
...
def train():
    ...
    for kf_idx, (remain_index, test_index) in enumerate(kf.split(idx_list)):
        if kf_idx in kf_indices:
            kf_tmp_output_path = os.path.join(tmp_output_path, "kfold_{}".format(kf_idx))
            ...
            cgcnnfz = CGCNNFeaturizer(
                task=args.task,
                distributed=distributed, n_works=args.n_works,
                disable_cuda=disable_cuda,
                save_idx=kf_tmp_output_path,
                output_path=kf_tmp_output_path,
                atom_init_fea=atom_features,
                use_batch=False, test=args.test,
                dropout_percent=0.5, batch_size=args.batch_size,
                ...
                lr_milestones=[800], lr=args.learning_rate,
                weight_decay=0.0,
                h_fea_len=32, pin_memory=args.pin_memory,
                n_conv=args.n_conv, n_h=1,
                del_checkpoint=False, use_idxes=True,
                train_set=train_index, val_set=val_index, test_set=test_index,
                atom_fea_len=64,
