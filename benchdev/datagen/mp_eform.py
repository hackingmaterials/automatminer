...
df = mpdr.get_dataframe(criteria={"formation_energy_per_atom": {"$lt": 2.5}},
                        properties=["material_id", "warnings"],
                        index_mpid=False)

print(df["warnings"].astype(str).value_counts())

structures = pd.DataFrame({"structure": [], "material_id": [], "formation_energy_per_atom": []})

for chunk in tqdm(chunks(range(len(df)), chunksize)):
    print(chunk[0], chunk[-1])
    mpids = df.loc[chunk[0]:chunk[-1], "material_id"].tolist()
    stchunk = mpdr.get_dataframe(criteria={"material_id": {"$in": mpids}}, properties=["structure", "material_id", "formation_energy_per_atom"], index_mpid=False)
...
# print(df.shape)
df["is_noble"] = [any([e.is_noble_gas for e in s.composition.elements]) for s in df["structure"]]
