...
class RunPipe(FireTaskBase):
    ...
    def run_task(self, fw_spec):
        ...
        fw_spec["base_save_dir"] = base_save_dir
        ...
        if problem_type == AMM_REG_NAME:
            ...
            pass_to_storage["f1"] = f1_score(true, test,
        ...
        pass_to_storage.update({"target": target,
...
class ConsolidatePipesToBenchmark(FireTaskBase):
    ...
    def run_task(self, fw_spec):
        ...
        problem_type = fw_spec["problem_type"]
        if problem_type == AMM_REG_NAME:
            scores_dict = {"rmse": copy.deepcopy(template_subdict),
                           "mae": copy.deepcopy(template_subdict),
                           "r2": copy.deepcopy(template_subdict)}
        elif problem_type == AMM_CLF_NAME:
            scores_dict = {"f1": copy.deepcopy(template_subdict),
...
class ConsolidateBenchmarksToBuild(FireTaskBase):
    ...
    def run_task(self, fw_spec):
        ...
        for bhash in benchmark_hashes:
            n_bdocs = benchmarks.find({"benchmark_hash": bhash, "build_id": build_id}).count()
            if n_bdocs != 1:
                raise ValueError("The number of benchmark docs with this build is {}, not 1!".format(n_bdocs))
            else:
                bench_doc = benchmarks.find_one({"benchmark_hash": bhash, "build_id": build_id})
                ...
                if dataset_name in build_doc:
                    raise ValueError("{} occurs more than once in the db (hash {})!".format(dataset_name, bhash))
                build_doc[dataset_name] = {k: bench_doc[k]["mean"] for k in score_types}
                build_doc[dataset_name]["mean_elapsed_time"] = bench_doc["elapsed_time"]["mean"]
                build_doc[dataset_name]["mean_test_sample_frac_retained"] = bench_doc["test_sample_frac_retained"]["mean"]
                build_doc[dataset_name]["mean_n_features"] = bench_doc["n_features"]["mean"]
