...
def parse_args():
    ...
    add_arg('-o', '--output_path',
            required=True, help='output path')
    add_arg('-i', '--input_file',
            default='../data/jdft2d.pickle.gz', help='input file')
    add_arg('-g', '--graph_file',
            default=None, help='graph file. If not None, load graphs from graph file, otherwise, create and save graphs')
    add_arg('-e', '--embedding_file',
            default=None, help='embedding file. If not None, transfer learning from embedding file, otherwise, not use transfer learning')
    add_arg('-t', '--type',
            default='regression', help='task type, regression or classification')
    add_arg('-p', '--property',
            default='gap pbe', help='target name')
    add_arg('-w', '--warm_start',
            default=None, help='warm start model file. If not None, warm start from file.')
    add_arg('-n', '--n_works',
            default=1, type=int, help='n works for dataloader')
    add_arg('-m', '--max_epochs',
            default=3000, type=int, help='max epochs')
    add_arg('-b', '--batch_size',
            default=128, type=int, help='mini batch size')
    add_arg('-l', '--loss',
            default='mse', help='loss function')
    add_arg('-lr', '--learning_rate',
            default=None, type=float, help='learning rate')
    add_arg('-cv', '--cv',
            default=5, type=int, help='cv number')
    add_arg('-kf', '--k_folds',
            default='0,1,2,3,4', help='kfold indexes list')
    add_arg('-r', '--radius',
            default='4.0', type=float, help='megnet parameter, radius: distance cutoff for neighbors')
    add_arg('-np', '--n_pass',
            default=2, type=int, help='megnet parameter, n_pass: number of recurrent steps in Set2Set layer')
    add_arg('-nb', '--n_blocks',
            default=3, type=int, help='megnet parameter, n_blocks: number of MEGNetLayer blocks')
    add_arg('-s', '--save_best_only',
            action='store_false', help='save best score models only or save all epoch models')
    add_arg('-norm', '--normalize',
            action='store_true', help='Normalize targets or not')
    add_arg('-dc', '--disable_cuda',
            action='store_true', help='disable cuda or not')
...
def train():
    ...
    cg = CrystalGraph(
        bond_convertor=GaussianDistance(np.linspace(0, radius + 1, 100),
                                        0.5), cutoff=radius)
    ...
    if graph_file is not None:
        ...
        valid_idx_list = [idx for idx, graph in valid_graph_dict.items()
        ...
        for idx in idx_list:
            try:
                ...
                valid_graph_dict[idx] = {'graph': graph,
    ...
    for fold, (train_val_idx, test_idx) in enumerate(kf.split(idx_list)):
        ...
        if warm_start is None:
            ...
            if learning_rate is None:
                learning_rate = 1e-3
            model = MEGNetModel(100, 2, nblocks=args.n_blocks, nvocal=95,
                                npass=args.n_pass, lr=learning_rate,
                                loss=args.loss,
                                graph_convertor=cg,
            ...
            # Model file
            model_list = [m_file for m_file in os.listdir(
                os.path.join(warm_start,  "kfold_{}".format(fold),
                             'model')) if m_file.endswith('.hdf5')]
            if args.type == "classification":
                model_list.sort(key=lambda m_file: float(
                    m_file.split('_')[3].replace('.hdf5', '')), reverse=False)
            else:
                model_list.sort(key=lambda m_file: float(
                    m_file.split('_')[3].replace('.hdf5', '')), reverse=True)
        ...
        # Train
        model.train_from_graphs(train_graphs, train_targets,
                                val_graphs, val_targets,
                                batch_size=args.batch_size,
                                epochs=args.max_epochs, verbose=2,
                                initial_epoch=initial_epoch,
                                use_multiprocessing=False
                                if n_works <= 1 else True,
                                workers=n_works,
