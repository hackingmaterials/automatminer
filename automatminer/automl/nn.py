...
from automatminer import DataCleaner, AutoFeaturizer
import pandas as pd
...
df = pd.read_pickle(
    "/Users/ardunn/alex/lbl/projects/common_env/dev_codes/automatminer/benchdev/testing/datasets/castelli.pickle.gz")

# soap = SOAP()
# df = soap.fit_featurize_dataframe(df, "structure")


# ep = ElementProperty.from_preset("matminer")
# df = ep.fit_featurize_dataframe(df, "structure")

target = "e_form"
af = AutoFeaturizer(cache_src="./test_castelli.json", preset="debug")
df = af.fit_transform(df, target)

dc_args = {"max_na_frac": 0.01, "feature_na_method": "mean",
           "na_method_fit": "drop", "na_method_transform": "mean"}
dc = DataCleaner(**dc_args)
...
class DNNBuilderSimple(adanet.subnetwork.Builder):
    ...
    def __init__(self, optimizer, layer_size, num_layers, learn_mixture_weights,
        ...
        Returns:
          An instance of `DNNBuilderSimple`.
        """
...
class DNNGeneratorSimple(adanet.subnetwork.Generator):
...
def train_and_evaluate(experiment_name,
    ...
    estimator = adanet.Estimator(
        ...
        subnetwork_generator=DNNGeneratorSimple(
...
if __name__ == "__main__":
    ...
    tf.estimator.predict(estimator)
